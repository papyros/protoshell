...
public:
    CustomAuthenticator(SessionManager *parent, const QJSValue &callback)
            : m_parent(parent), m_callback(callback), m_succeded(false)
    {
...
SessionManager::SessionManager(QObject *parent)
        : QObject(parent), m_authenticatorThread(new QThread(this)), m_authRequested(false),
          m_authenticator(new Authenticator), m_loginManager(new LoginManager(this, this)),
          m_powerManager(new PowerManager(this)), m_screenSaver(new ScreenSaver(this)),
          m_idle(false), m_locked(false)
{
...
bool SessionManager::isIdle() const { return m_idle; }
...
bool SessionManager::isLocked() const { return m_locked; }
...
bool SessionManager::canRestart() { return m_powerManager->capabilities() & PowerManager::Restart; }

bool SessionManager::canSuspend() { return m_powerManager->capabilities() & PowerManager::Suspend; }
...
void SessionManager::powerOff() { m_powerManager->powerOff(); }

void SessionManager::restart() { m_powerManager->restart(); }

void SessionManager::suspend() { m_powerManager->suspend(); }

void SessionManager::hibernate() { m_powerManager->hibernate(); }

void SessionManager::hybridSleep() { m_powerManager->hybridSleep(); }

void SessionManager::lockSession() { m_loginManager->lockSession(); }
...
void SessionManager::activateSession(int index) { m_loginManager->switchToVt(index); }
...
void SessionManager::cancelShutdownRequest() { Q_EMIT shutdownRequestCanceled(); }
