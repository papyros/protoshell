...
Application::Application(QObject *parent) : QObject(parent), m_failSafe(false), m_started(false)
{
    ...
    // Fail safe mode
    connect(m_homeApp, &HomeApplication::objectCreated, this, &Application::objectCreated);
...
void Application::setUrl(const QUrl &url) { m_url = url; }
...
void Application::startup()
{
    ...
    // Session interface
    m_homeApp->setContextProperty(QStringLiteral("SessionInterface"), m_sessionManager);
...
void Application::unixSignal() { QCoreApplication::quit(); }
...
void Application::objectCreated(QObject *object, const QUrl &)
{
    ...
    if (m_failSafe) {
        ...
        qWarning("A fatal error has occurred while running Papyros Shell, but the error "
...
StartupEvent::StartupEvent() : QEvent(StartupEventType) {}
