...
void NotificationServer::closeNotification(uint id) { adaptor->closeNotification(id); }
...
Notification *NotificationServer::notify(QString app_name, uint replaces_id, QString app_icon,
                                         ...
                                         QVariantMap hints, int expire_timeout, int progress)
{
    if (expire_timeout == 0) {
        expire_timeout = 2500;
    }
    ...
    Notification *notification =
            new Notification(app_name, (replaces_id == 0 ? availableId++ : replaces_id), app_icon,
                             summary, body, actions, hints, expire_timeout, progress);
